...
class HitConverter:
        ...
        if self.pcluster_config.cluster_model != ClusterModel.HIT:
            ...
            if scheduler != "slurm":
                ...
                if sit_cluster_section.get_param_value("placement") == "cluster":
                    LOGGER.log(
                        log_level,
                        "Warning: 'placement = cluster' is not supported in the Heterogeneous Instance Type model "
    ...
    def _store_original_sections(self):
        ...
        for section_type in mappings.CLUSTER_SIT_NESTED_SECTIONS + mappings.GLOBAL_FILE_SECTIONS:
    ...
    def _restore_original_sections(self, hit_cluster_section):
        ...
        for section in self._cluster_nested_sections:
            if section.key in [section.get("key") for section in mappings.ALWAYS_PRESENT_SECTIONS]:
                # remove default sections
                self.pcluster_config.remove_section(section.key, "default")

            if section.key in [section.get("key") for section in mappings.CLUSTER_SIT_NESTED_SECTIONS]:
